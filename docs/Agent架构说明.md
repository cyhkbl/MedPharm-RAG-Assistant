...
1. **引入 Rerank 二次排序**：在 RRF 后增加中文医学 reranker，提高 Top5 上下文质量。
2. **支持增量更新**：新增教材时只对新增内容抽取、对齐和索引，避免全量重建。
3. **多模态图表解析**：引入 OCR 和视觉模型，解析教材中的图、表和流程图。
4. **专业词表增强**：接入医学术语词典，提高同义词、缩写和中英文映射准确率。
5. **人工审核闭环**：将教师覆盖决策沉淀为偏好规则，提高下一轮整合质量。
