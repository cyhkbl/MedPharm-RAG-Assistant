...
**实测数据**（详见 `docs/P2技术报告.md`）：

| chunk_size | 检索方式 | 命中率@5 | 平均响应时间 | Token/查询 |
|-----------|----------|----------|-------------|-----------|
...
结论：600字 chunk + 混合检索在命中率和效率之间取得最优平衡。
...
| Prompt | 防幻觉策略 | Few-shot |
|--------|-----------|----------|
| KNOWLEDGE_EXTRACTION_PROMPT | 要求"只抽取原文明确出现的内容"，禁止编造 | 1 条示例（炎症） |
| RELATION_EXTRACTION_PROMPT | 限定 relation_type 枚举，只输出有教学价值的关系 | 1 条示例（静息电位→动作电位） |
| EQUIVALENCE_JUDGMENT_PROMPT | 明确判断标准（同义词=等价，上下位≠等价） | 2 条示例（白细胞=白血细胞，动脉≠静脉） |
| RAG_ANSWER_PROMPT | "只基于上下文"+"未找到则拒答"+"必须引用来源" | 隐式（格式约束） |
| DIALOGUE_SYSTEM_PROMPT | "保持医学准确性"+"指出需人工确认的内容" | 无（开放式对话） |
...
系统支持知识图谱四种视图展示（`KnowledgeGraph.tsx` 中 ViewMode 类型）：

1. **力导向图**（force）：展示知识点关联密度，节点大小=频次，颜色=教材来源。
2. **树状图**（tree）：按教材→章节→知识点层级展示。
3. **对比视图**（compare）：整合前后两张力导向图并排对比。
4. **桑基图**（sankey）：用 ECharts type:"sankey" 展示整合决策中 merge 操作的知识流向——左侧为整合前的 affected_nodes，右侧为合并后的 result_node。

多视图让教师既能看全局结构，也能检查具体整合链路。

### 11.2 混合检索 + RRF 融合

系统不是简单纯向量 RAG，而是将向量检索和 BM25 检索结合（`retriever.py` 中 HybridRetriever 类）。医学问答中大量问题依赖精确术语，RRF 融合（k=60）可以显著提高召回稳定性。

### 11.3 双重语义对齐

Embedding 用于快速召回候选（余弦相似度 ≥0.85），LLM 用于精确判断等价关系（`aligner.py`）。这种"粗召回 + 精验证"的设计兼顾速度和准确性。

### 11.4 自建 RAG Benchmark

系统设计了 20-50 道医学教材评测题（`utils/benchmark.py`），可自动评估：
...
LLMClient 中的 TokenStats 类（`core/llm/client.py`）实时记录每次 LLM 调用的 token 消耗：
- 输入 token、输出 token、推理 token 分别计数
- 按模型分类统计
- API 端点 `GET /api/stats/tokens` 暴露统计数据
- 前端 Report 组件中以卡片形式可视化展示

### 11.6 教师多轮对话迭代优化

教师可以通过对话直接修改整合决策（`api/dialogue.py` 中 `_try_override_decision`）：
- LLM 意图识别：分析用户消息是否包含修改意图
- 关键词匹配：在 decisions 中查找匹配的知识点
- 自动执行：修改对应决策的 action 并持久化
- 例如："把《生理学》里的炎症保留" → 自动将该决策改为 keep
...
基于知识图谱中的 prerequisite 关系，系统可以自动推荐学习路径（`core/learning_path.py`）：
- **拓扑排序**：从入门知识点到高级知识点的学习顺序
- **目标导向**：指定目标知识点后，自动推荐所有前置知识路径
- **可视化**：前端使用力导向图展示学习路径，绿色=起点，红色=终点

### 11.8 知识点难度评估

系统基于两个维度自动评估知识点难度（`assess_difficulty` 函数）：
- **前置依赖数**：prerequisite 入度越高 → 需要先修越多 → 难度越高
- **频次修正**：跨教材高频出现的基础概念难度降级
- **三级分类**：入门（无前置）、中级（1-2个前置）、高级（3+个前置）
- 前端以表格+颜色编码展示（绿=入门，黄=中级，红=高级）

### 11.9 PDF 报告导出

整合报告支持 PDF 格式导出（`GET /api/report/pdf`）：
- 使用 reportlab 生成标准 A4 PDF
- 包含整合概览、决策摘要、知识图谱统计、Token 消耗
- 前端一键导出按钮

### 11.10 PDF 章节字号自适应阈值

PDF 解析器中的 `_heading_threshold` 函数（`core/parser/pdf_parser.py`）不是使用固定字号阈值，而是按整本书自适应计算：
- 计算所有字号的平均值 × 1.25
- 取 80 分位数
- 两者取 max 作为标题阈值

这种自适应设计确保了不同排版风格的教材都能正确识别章节标题。

### 11.11 uuid5 稳定 ID 生成

知识点 ID 和决策 ID 使用 `uuid5(NAMESPACE_URL, seed)` 生成（`aligner.py`、`integrator.py`）：
- 相同输入 always 产生相同 ID（幂等可复现）
- 避免了随机 ID 导致的不可重现问题
- 便于调试和跨会话数据比对

### 11.12 流式分块写盘

文件上传使用 1MB chunk 流式写盘（`api/upload.py`），超 500MB 文件返回 413：
- 避免大文件一次性加载到内存
- 支持 826MB 的 7 本教材批量上传
- 逐页解析避免 OOM

### 11.13 页眉页脚自动检测

PDF 解析器中的 `_is_header_footer` 函数通过双维度判定页眉页脚：
- **频率维度**：出现在 ≥ 50% 的页面中
- **位置维度**：位于页面顶部 12% 或底部 12%
- 两个条件同时满足才判定为页眉页脚并过滤

### 11.14 AI 测验自动生成

基于整合知识点，LLM 自动生成不同难度的测验题（`core/teaching.py` 中 `generate_quiz`）：
- 题型：选择题（60%）+ 简答题（40%）
- 难度分布：简单 30%、中等 50%、困难 20%
- 每题包含：题目、选项、正确答案、解析、涉及知识点
- 前端支持点击"查看答案"交互
- 兜底机制：LLM 失败时自动生成基于定义的简答题

### 11.15 教学大纲自动生成

基于知识图谱自动生成完整教学大纲（`generate_teaching_outline`）：
- 10-15 个教学单元，总学时 48-72
- 每单元：标题、学时、教学目标、重点知识点、前置要求
- 按照从基础到临床的逻辑顺序排列
- 直接服务教师备课场景

### 11.16 教材覆盖率热力图

教材×知识点分类的热力图矩阵（`compute_coverage_matrix`）：
- ECharts heatmap 可视化
- 颜色深浅表示覆盖数量
- 自动识别知识盲区（某教材缺少某分类的知识点）
- 帮助教师快速发现教材间的互补与缺失
